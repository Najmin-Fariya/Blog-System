...
<div class="slider display-table center-text">
    <h1 class="title display-table-cell"><b>{{ $category->categoryName }}</b></h1>
</div><!-- slider -->
                                    ...
                                    <li>
                                        ...
                                        @else
                                            <a href="javascript:void(0);" onclick="document.getElementById('favourite-form-{{ $post->id}}').submit();"
                                               ...
                                               >
                                            <i class="ion-heart"></i>{{ $post->favourite_to_users->count() }}</a>
                                            <form action="{{ route('category.posts',$post->id)}}" id="favourite-form-{{ $post->id}}" method="POST" style="display: none;">
                                            @csrf
                                            </form>
                                        @endguest
